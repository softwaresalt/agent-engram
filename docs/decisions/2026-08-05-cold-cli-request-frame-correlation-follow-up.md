...
- Final source revision: `ca398bd6`. This includes a non-live-only JSON capture
  remediation made after attempt two. The remediation has deterministic build
  and test coverage but no additional runtime claim.
...
- Aggregate supervisor: five minutes including startup, evidence collection,
  graceful shutdown, idle fallback, endpoint verification, and temp cleanup.
...
| Aggregate elapsed, including cleanup | 8,438 ms | 7,770 ms |
...
| Cleanup | PID dead, pipe unreachable, temp removed | PID dead, pipe unreachable, temp removed |
...
emit the event as JSON. Non-live verification passed, including the focused
deterministic tests, pedantic Clippy, formatting, and release compilation.
Because the live cap was already exhausted, this final format change was not
executed against another cold daemon and does not close the runtime blocker.
...
pipe was unreachable, and each temporary workspace was removed before its
aggregate supervisor ended. Repository daemon PID `16084` and its workspace
binding remained observation-only and healthy.
