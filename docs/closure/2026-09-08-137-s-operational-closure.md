...
| `F35EA0E6` | high | **Closure-PR review follow-up**: Copilot review on PR #389 raised whether `137-S`'s `archived_status: done` (manual safe-close) satisfies queued shipment `142-S`'s blocks-dependency eligibility check, citing `.github/agents/_ship.agent.md:787-790` and `.github/instructions/backlogit.instructions.md:63-80` which literally require `shipped`. Same open design question already captured as `77A4E71C` from `135-S`'s closure, now recurring one link further down the chain; empirically the chain (`134-S` through `137-S`) has not actually been blocked by this in practice, but the underlying mechanism is unconfirmed. Out of scope per P-021 C1 for this closure PR. |

All four are `requires_deliberation: true` (or already so recorded) and
