...
| Aggregate bound | 8,438 ms and 7,770 ms, each including cleanup |
| Cleanup | PIDs `16360` and `29700` dead; both pipes unreachable; temp workspaces removed |
...
as JSON; deterministic tests, formatting, pedantic Clippy, and release
compilation pass, but the hard live cap prevents another runtime claim.
