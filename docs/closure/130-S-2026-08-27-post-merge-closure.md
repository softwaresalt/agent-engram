...
|---|---|---|---|---|
| Readiness-timeout sessions recover without restart (heuristic) | Count `failure_class: readiness_timeout` records in `<workspace>/.engram/diagnostics/shim-startup-failures.jsonl` over the observation window (each record is a manual-investigation trigger, not itself a pass/fail signal — the file has no corresponding recovery event); separately, the CI contract test `contract_shim_stdio_initialize::shim_recovers_after_timed_out_daemon_later_becomes_ready` on every `main` build is the actual pass/fail baseline. **Caveat**: the diagnostics record schema carries only `timestamp`, `binary_version`, `failure_class`, and `message` — no PID or session-correlation key — so a record cannot be joined 1:1 to a specific client-log session when multiple shim processes run concurrently; this is a population-level heuristic, not a per-record proof, until a correlation key is added | CI: 100% of `main`-build recovery-test runs pass. Field: 0 confirmed reports of a session stuck on `readiness_timeout` after the daemon reports ready (there is no comparable field/CI rate to diverge — investigate each diagnostics record individually) | Any CI regression on the recovery test, or any confirmed field/support report describing a session stuck on `readiness_timeout` after the daemon reports ready | Repository maintainer (`softwaresalt`) |
| Terminal daemons misreported as recoverable | No durable log captures this today: `recoverable` only appears in the live `tools/call` MCP response payload (`src/shim/transport.rs:297-305`), not in `shim-startup-failures.jsonl` (timestamp/build/class/message only). Until `138-F` adds durable terminal-outcome recording, verify by manual reproduction: pair a captured `tools/call` response showing `recoverable: true` with the daemon's `_health` response that produced it (e.g. a `VersionMismatch`/protocol-incompatible reply arriving after the readiness deadline), and record both side by side | N/A (known, unfixed gap — this is the exact defect `138-F` closes) | Any reproduction, or field/support report, showing `recoverable: true` for a confirmed protocol-incompatible daemon | Repository maintainer (`softwaresalt`) |
