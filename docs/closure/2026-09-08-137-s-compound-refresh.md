...
|---|---|---|
| `docs/compound/workflow-issues/backlogit-shipment-ship-non-terminating-large-covering-feature-2026-09-06.md` | **update** | Appended a dated addendum recording that `137-S` reused this entry's existing `135-S` evidence to justify skipping `backlogit shipment ship` entirely, without re-attempting the command — this is *not* a second independently confirmed occurrence of the non-termination symptom (the command was never invoked for `137-S`). The generic `backlogit move --status shipped` CLI rejection *was* independently re-confirmed. Citations preserved; no content removed. |
