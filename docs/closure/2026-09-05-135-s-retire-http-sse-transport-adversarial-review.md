...
## Third Copilot review round (FIXED)

### 8. Checked-in `.github/copilot-instructions.md` and `.claude/instructions.md` were not regenerated
**Severity: MAJOR | Confidence: Copilot round 3 (PR comment 3943145889) | P1 — fixed directly**

The engram-managed marker sections (`<!-- engram:start -->`/`<!-- engram:end
-->`) in this repository's own checked-in `.github/copilot-instructions.md`
and `.claude/instructions.md` were generated by a real `engram install` run
at some point in the past and committed. 142.025-T's `templates.rs` fix only
changes what a *future* install run produces — it does not retroactively
update already-generated, already-committed content. This left the two
files this repository's own Copilot/Claude integrations actually read still
advertising the retired `http://127.0.0.1:7437/mcp` endpoint, directly
contradicting the change. `.github/copilot-instructions.md` also had a
stale `axum | HTTP server for legacy SSE transport` row in its (separately
autoharness-generated) technology table.

**Fix**: ran the actual `engram install --hooks-only` command against this
repository, which regenerated exactly the marker-delimited section in both
files via the real `apply_markdown_hook` code path (byte-for-byte
consistent with what `templates.rs` now produces, not manually
reconstructed); `.mcp.json` had no diff (already correct, add-if-absent).
Manually removed the stale `axum` technology-table row (outside the
marker-managed section, so not touched by the install regeneration). This
is a same-contract-surface completion of 142.025-T, not a scope expansion —
it is required to make the already-approved template change
non-self-contradictory within this same repository.
