...
4. Use phase-specific disk admission instead of multiplying the protected core target. Before U1 read-only discovery, require workspace-volume free bytes of at least `8 GiB`; this fixed floor covers metadata, advisory-db, logs, and bounded static-inspection artifacts and does not count the existing core `target/`. Before U2 or U3 execution, measure incremental footprint `I` as the allocated bytes expected for the isolated run root, candidate source, new locked-package source/cache material, synthetic data, and logs; exclude the existing core target and pre-existing caches. Require free bytes of at least `max(20 GiB, ceil(1.5 * I) + 2 GiB)`, record the inputs and result, and recheck before each execution phase. Stop if the estimate is unavailable, the fixed floor is not met, or free space drops below the applicable threshold.
