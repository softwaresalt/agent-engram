...
* Local tools required by this skill must be available in PATH, including the configured quality gates `cargo fmt --all -- --check`–`cargo test`; if formatting fixes are applied during remediation, `cargo fmt --all` must also be available
