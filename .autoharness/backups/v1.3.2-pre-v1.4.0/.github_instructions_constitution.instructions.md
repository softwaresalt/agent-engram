...
#![forbid(unsafe_code)]. clippy pedantic with deny warnings. All fallible operations return `Result<T, EngramError>`. `EngramError` wraps domain-specific sub-errors via `#[from]`; each variant maps to a u16 error code. Error codes: 1xxx (Workspace), 2xxx (Hydration), 4xxx (Query), 5xxx (System), 7xxx (Code Graph). Map external errors via `From` impls or `.map_err()` — never `unwrap()` or `expect()`..
...
directory structure (tests/contract/ (MCP tool contract verification), tests/integration/ (end-to-end flows), tests/unit/ (isolated logic + proptest)) MUST be maintained. All tests MUST
...
| Documentation   | /// doc comments on all public items; //! on module files                                            |
...
|---|---|---|
| I. Safety-First Language | MUST | CI quality gates (`cargo clippy -- -D warnings -D clippy::pedantic`, `cargo build`) |
...
**Version**: 1.0.0 | **Ratified**: 2026-04-24 | **Generated by**: autoharness
