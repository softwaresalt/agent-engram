...
use engram::services::dehydration::{self, SCHEMA_VERSION};
use engram::services::hydration::{hydrate_into_db, hydrate_workspace};
use engram::tools;
...
// ─── T058: Integration test for hydration from .engram/ files ─────────────────

#[test]
async fn hydrate_from_engram_files_loads_tasks_into_db() {
...
",
    ...
    fs::write(
        engram_dir.join("graph.surql"),
        r"-- Generated by Engram
...
#[test]
async fn hydrate_empty_workspace_creates_engram_dir() {
    ...
    assert!(workspace.path().join(".engram").exists());
}
...
",
    ...
    assert!(
        result
            .files_written
            .contains(&".engram/tasks.md".to_string())
    );
    assert!(
        result
            .files_written
            .contains(&".engram/graph.surql".to_string())
    );
    ...
    let version = fs::read_to_string(engram_dir.join(".version")).expect("read version");
    assert_eq!(version, SCHEMA_VERSION);

    // Lastflush written
    assert!(engram_dir.join(".lastflush").exists());
}
...
",
    ...
    let flushed = fs::read_to_string(engram_dir.join("tasks.md")).expect("read tasks.md");
    assert!(
...
async fn flush_state_fails_on_stale_files_in_fail_mode() {
    ...
    assert_eq!(payload.error.code, engram::errors::codes::STALE_WORKSPACE);
}
...
async fn shutdown_flush_dehydrates_active_workspace() {
    ...
    // Verify .engram/ files were written
    let engram_dir = workspace.path().join(".engram");
    assert!(
    ...
    assert!(
        engram_dir.join(".version").exists(),
        ".version should exist"
    );
    assert!(
        engram_dir.join(".lastflush").exists(),
