...
if _sidecar_enabled "backlogit" && command -v backlogit >/dev/null 2>&1; then
	# Anchor the sidecar syncs to the repository directory so backlogit and
	# engram resolve this workspace regardless of the caller's location. The
	# subshell keeps the caller's working directory unchanged.
	(cd "$script_dir" && backlogit sync) || echo "Warning: backlogit sync failed (non-fatal)" >&2
fi
...
if _sidecar_enabled "graphtor-docs"; then
	...
	if [[ -n "$graphtor_docs_exe" ]]; then
		(cd "$script_dir" && "$graphtor_docs_exe" sync) || echo "Warning: graphtor-docs sync failed (non-fatal)" >&2
	fi
fi
