...
Single-binary MCP daemon distributed via GitHub Releases. Development follows conventional commits with cargo-release automation and git-cliff changelog generation. Feature branches use number-prefixed naming (e.g., 016-mcp-sandbox-policy-engine).
...
#![forbid(unsafe_code)]. All clippy warnings are hard errors (-D warnings -D clippy::pedantic); unsafe code is forbidden at the crate level.
All errors must be propagated via Result<T, EngramError> using the ? operator. The unwrap_used and expect_used lints are denied at workspace level..
...
pass via `cargo dev-test` before any code is merged.
...
into `docs/research/` as part of its post-merge closure protocol. The
