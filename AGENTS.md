...
Agent Engram is a Rust MCP daemon developed with a structured two-agent pipeline. Stage manages triage-to-harvest (stash, deliberation, planning, review gating). Ship manages build-to-release (claiming, building, testing, reviewing, CI fixing, PR lifecycle). Both agents use circuit-breaker protocols and concurrency controls. Work is tracked via backlogit (MCP-integrated). Quality gates enforce fmt → clippy → test before merge.
...
`#![forbid(unsafe_code)]` — no unsafe code anywhere. All clippy pedantic warnings are hard errors; `unwrap()` and `expect()` are denied at workspace level.
All errors must be propagated via Result<T, EngramError>; never use unwrap() or expect() on fallible paths.
