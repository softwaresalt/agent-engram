# Data Model: T-Mem Core MCP Daemon
...
T-Mem uses a **graph-relational** data model where core entities (Spec, Task, Context) are connected via typed edges (implements, depends_on, relates_to). This enables both tabular queries and graph traversals.
...
## File Format: `.tmem/graph.surql`

Graph relationships are serialized as SurrealQL:

```surql
-- Generated by T-Mem. Do not edit manually.
...
Version stored in `.tmem/.version` file.
