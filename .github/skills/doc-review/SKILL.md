...
When `anthropic` and `claude-sonnet-5` are
...
* Commands that no longer appear in the project toolchain
  (`cargo build`, `cargo dev-test`, `cargo clippy --all-targets -- -D warnings -D clippy::pedantic`).
...
1. Read `anthropic` and `claude-sonnet-5`.
2. If both are non-empty: use the alternate provider/family for this review
   pass. Log `[DOC-REVIEW] Using alternate model: claude-sonnet-5
   (provider: anthropic)`.
...
When `anthropic` / `claude-sonnet-5` are set:
