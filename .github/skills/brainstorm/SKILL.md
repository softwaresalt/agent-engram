...
`docs/research/{YYYY-MM-DD}-{slug}-requirements.md`.
...
* **Queue** → link a backlog entry for later pursuit. When the `backlogit`
  ...
  instead — a bullet carrying the date, topic, the `docs/research`
...
* **None** → leave the artifact under `docs/research/` unlinked.
...
doc_type: spec
source: "docs/research/{YYYY-MM-DD}-{slug}-requirements.md"
date: "YYYY-MM-DD"
source_stash_ids: []
source_research:
