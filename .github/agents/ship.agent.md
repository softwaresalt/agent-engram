...
7. **Source artifact cleanup** (backlogit only): When the `backlogit` capability pack is installed, retire source-artifact follow-up heuristics and use only verifiable links supported by the installed backlog registry.
   * For each shipped top-level item in scope (feature or chore), read `custom_fields.source_stash_id`. If present, record the stash ID in the closure artifact and append a concise `backlogit_append_comment` note to the shipped item so the stash ID is available for manual retirement. No automated stash-retirement operation exists in the installed registry — record the ID for traceability only. If the field is absent, skip and log it.
   * For each shipped top-level item in scope (feature or chore), read `custom_fields.source_deliberation_id` if present, and also inspect `references` for deliberation artifacts (for example `docs/decisions/*deliberation*.md` or reviewed plan paths). If deliberation references exist, record those paths in the closure artifact and append a concise `backlogit_append_comment` note to the shipped item so the deliberation follow-up remains traceable. If no such references exist, skip and log it.
   * Note: `backlogit_stash_remove` and `backlogit_archive_item` are NOT in the installed registry — use `backlogit_append_comment` for all source-artifact traceability instead.
   * After processing the full shipped scope, record the noted stash IDs, deliberation references, and skipped entries in the closure artifact's `Source artifact cleanup` section so the closure report remains the traceable system of record.
   * When the `agent-intercom` capability pack is installed, broadcast `[SHIP] Source artifacts recorded for follow-up: {stash_count} stash, {delib_count} deliberation refs`.
