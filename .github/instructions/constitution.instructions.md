...
`#![forbid(unsafe_code)]` — no unsafe code anywhere. All clippy pedantic warnings are hard errors; `unwrap()` and `expect()` are denied at workspace level. All errors must be propagated via Result<T, EngramError>; never use unwrap() or expect() on fallible paths.
...
**Version**: 1.0.0 | **Ratified**: 2026-04-13 | **Generated by**: autoharness
