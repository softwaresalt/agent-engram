...
directory structure (Three-tier test architecture: contract tests in tests/contract/ (MCP tool response verification), integration tests in tests/integration/ (end-to-end with real DB/IPC), unit tests in tests/unit/ (isolated logic with proptest)) MUST be maintained. All tests MUST
pass via `cargo dev-test` before any code is merged. Steps: write test,
